    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("reload")) {
            if (sender instanceof Player) {
                if (!sender.hasPermission("autoblock.reload")) {
                    sender.sendMessage(formatMessage(prefix + "§cYou don't have permission to use this command."));
                    return true;
                }
            }
            plugin.reloadConfig();
            plugin.loadConversionRules();
